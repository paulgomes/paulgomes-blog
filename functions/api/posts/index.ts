...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const postsSelect = `
    SELECT
      ...
      categorias,
      is_featured,
      CASE WHEN meta_title IS NOT NULL AND meta_title != '' AND meta_description IS NOT NULL AND meta_description != '' AND focus_keyword IS NOT NULL AND focus_keyword != '' THEN 1 ELSE 0 END AS seo_ok,
      status,
      published_at,
      updated_at,
