...
// GET  /api/brands  -> { title, items: [{ id, name, url, position }] }
// POST /api/brands  -> { name, url? }  cria marca (position = max+1)
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const [brands, titleRow] = await Promise.all([
    env.DB.prepare('SELECT id, name, url, position FROM brands ORDER BY position ASC, created_at ASC').all<{ id: string; name: string; url: string | null; position: number }>(),
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const now = Date.now();
  await env.DB
    .prepare('INSERT INTO brands (id, name, url, position, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)')
    .bind(id, name, url, position, now, now)
    .run();
